...
namespace
{
    ...
    /**
     * @brief チェックサムのための関数
     * *dataの型に関わらずこれで受けて、この内部でuint8_tにキャストして特殊化した方の実装で処理する。
     * ユーザからキャストを隠蔽できる。 みかん本の287p
     */
    template <typename T>
    ...
    {
        // 1byteに足すどんどん足していく。オーバーフローするが、チェックサム用途なので下位1バイトにしか興味がなく問題ない。
        uint8_t sum = 0;
...
namespace acpi
{
    ...
    {
        // this->signatureはNUL文字で終端されていないのでstrcmpが利用できない、文字数を指定するstrNcmpを使う。
        ...
        if (this->revision != 2)
        {
            // 参照したいxsdt_addressはリビジョン2の場合のみ存在
    ...
    {
        // flagsの8bit目を取り出す。カウンタが24bit幅だと偽、32bit幅だと真になる。
        ...
        if (!pm_timer_32)
        {
            // 24bit幅の場合はendも24bit幅に制限する
            end &= 0x00ffffffu;
        }

        if (end < start)
        {
            // endがオーバーフローしてstartより小さくなった場合の対応（みかん本の図12.2）
    ...
    void Initialize(const RSDP &rsdp)
    {
        ...
        for (size_t i = 0; i < xsdt.Count(); i++)
        {
            ...
            if (entry.IsValid("FACP")) // FADTのシグネチャはFACP
            {
